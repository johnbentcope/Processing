...
void draw() {
  ...
  if (frameCount < 180 && frameCount % 2 == 0) {
    rack.add( new Boxx(boxes.get( floor( random( boxes.size() ) ) ).dims, boxes.get( floor( random( boxes.size() ) ) ).isDw, true ));
  }
  ...
  pushMatrix();
  translate(30,0,0);
  rotateZ(2*(frameCount)*TWO_PI/360);
  rotateX(2*(frameCount)*TWO_PI/360);
  boxes.get(8).render();
  popMatrix();
